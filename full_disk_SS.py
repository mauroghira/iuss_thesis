...
# ─── Parametri fissi (stessi del notebook) ────────────────────────────────────
mm  = 1       # modo azimutale
hor = 0.05    # aspect ratio H/r
alpha_visc = 0.1  # viscosità fiduciale (solo per frontiere S&S)
...
def ss_boundaries(a, B00, M=M_BH, alpha=alpha_visc):
    """
    Calcola le frontiere radiali tra le tre zone di Shakura-Sunyaev
    in unità di r_g, usando le formule originali S&S (1973).

    Il tasso di accrescimento adimensionale Ṁ/Ṁ_Edd è stimato da B00
    tramite l'equilibrio di pressione magnetica all'orizzonte:
        B²/(8π) ~ ρ v_ff²  →  Ṁ/Ṁ_Edd ∝ B00² r_H² / (M c²)

    Parameters
    ----------
    a    : float   spin adimensionale
    B00  : float   B₀ all'orizzonte [unità arbitrarie coerenti con Sigma0]
    M    : float   massa BH in M_sun
    alpha: float   parametro di viscosità (default 0.1)
    ...
    # proxy per Mdot/Mdot_Edd
    mdot = np.clip(B00**2 * rH**2 / (M * 1e6), 1e-6, 10.0)

    # Formule S&S (1973) in r_g  (Novikov-Thorne corrected, approssimate)
    r_AB = 160  * alpha**(2/21) * mdot**(16/21) * (M/1e6)**(2/21)
    r_BC = 5e4  * alpha**(2/3)  * mdot**(2/3)   * (M/1e6)**(1/3)

    # Clamp: le frontiere devono essere fisicamente ordinate
    r_AB = float(np.clip(r_AB, rISCO * 1.5, 80.0))
    r_BC = float(np.clip(r_BC, r_AB  * 2.0, 600.0))
...
def sound_speed_disk(r, a, M=M_BH, hr=hor):
...
def solve_k_disk(r, a, B0, Sigma, c_s, m=mm, M=M_BH):
    """
...
def compute_beta_disk(r, a, B0, Sigma, c_s, M=M_BH, hr=hor):
...
def compute_full_disk_profile(a, B00, Sigma0, M=M_BH,
    ...
    ----------
    a            : float   spin (−1, 1)
    B00          : float   B₀ all'orizzonte degli eventi
    Sigma0       : float   Σ all'ISCO
    ...
    df : DataFrame con colonne:
         r, zone, B0, Sigma, c_s, k_cm [1/cm], k [1/rg], kr,
         beta, dQdr,
         k_valid (bool: 0.1 ≤ kr ≤ 10),
         beta_valid (bool: β ≤ 1),
         shear_valid (bool: dQ/dr > 0),
         aei_valid (bool: tutti e tre)
    meta : dict con r_H, r_ISCO, r_AB, r_BC, norms
    """
    rH    = r_horizon(a)
    rISCO = r_isco(a)
    r_AB, r_BC = ss_boundaries(a, B00, M)
    ...
    cs_arr    = sound_speed_disk(r_arr, a, M)
    ...
    # Beta e shear
    beta_arr  = compute_beta_disk(r_arr, a, B0_arr, Sigma_arr, cs_arr, M)
...
def plot_full_disk_profiles(df, meta, figsize=(16, 12)):
    """
...
def find_rossby_matches_SS(param_dict,
    ...
    """
    from setup import create_param_grid, Rg_SUN, M_BH
    ...
    for ia, a_val in enumerate(a_vals):
        ...
        for iB, B00_val in enumerate(B00_vals):
            for iS, S0_val in enumerate(S0_vals):
                ...
                r_AB, r_BC = ss_boundaries(a_val, B00_val, M)
                ...
                cs_flat = sound_speed_disk(r_flat, a_val, M)

                k_flat   = solve_k_disk(r_flat, a_val, B0_flat, S_flat, cs_flat, m=m, M=M)
                k_flat_rg = k_flat * Rg
                beta_flat = compute_beta_disk(r_flat, a_val, B0_flat, S_flat, cs_flat, M)
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8.  ESEMPIO D'USO  (eseguibile come script standalone)
# ═══════════════════════════════════════════════════════════════════════════════
...
if __name__ == '__main__':
    ...
    df_disk, meta = compute_full_disk_profile(
        a=a_test, B00=B00_test, Sigma0=Sigma0_test,
        check_norm=True
    )

    plot_summary_table(df_disk, meta)

    fig1 = plot_full_disk_profiles(df_disk, meta)
    fig2 = plot_aei_validity_map(df_disk, meta)
    plt.show()

    # ── Griglia parametri (compatibile con il notebook) ───────────────────────
    params = {
        'a':      [-0.9, 0.9, 10],
        'r':      [1, 1000, 50],
        'B00':    [1e-3, 1e8, 12],
    ...
    print("\nRicerca soluzioni AEI con modello SS unificato...")
    df_matches = find_rossby_matches_SS(
        params, check_k=True, check_beta=True, check_shear=True
    )
    print(f"Soluzioni trovate: {len(df_matches)}")
    if len(df_matches):
        print(df_matches[['a','r','B00','Sigma0','zone','k','kperr','beta']].head(10))
