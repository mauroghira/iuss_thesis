...
# parametro. A differenza delle bande "fuse" sull'intero span di
# frequenza (che per nu_per, nu_phi saturano quasi tutto lo spazio, si
# veda discussione precedente), a nu0 FISSATO il vincolo nu_X(R,a,M)=nu0
# e' una singola equazione, quindi una superficie di codimensione 1 in
# (a,R,M): la sua proiezione su ciascuna coppia di assi resta un anello
# sottile anche dopo marginalizzazione, non un'area piena.
#
# Costruzione (per derivazione si veda la discussione in chat):
...
#     (per continuita' di g_X in R, l'immagine e' un intervallo:
...
# --------------------------------------------------------
##########################################################
def plot_projection_aR(ax, chgrid, nu0, mass_range, color, ls="-", alpha=0.35, label=None):
    ...
    if M_lo == M_hi:
        ax.contour(chgrid.A, chgrid.R, chgrid.G, levels=[M_lo*nu0],
                   colors=[color], linewidths=1.1, linestyles=ls)
    else:
    ...
    if label:
        ax.plot([], [], color=color, lw=4, alpha=alpha, ls=ls, label=label)
    ax.set_yscale("log")
    ax.set_xlabel("a"); ax.set_ylabel(r"R [$R_g$]")


def plot_projection_aM(ax, chgrid, nu0, color, ls="-", alpha=0.35, label=None):
    ...
    ax.plot(chgrid.a_vals[valid], M_lo[valid], color=color, lw=0.8, ls=ls)
    ax.plot(chgrid.a_vals[valid], M_hi[valid], color=color, lw=0.8, ls=ls)
    if label:
        ax.plot([], [], color=color, lw=4, alpha=alpha, ls=ls, label=label)
    ax.set_yscale("log")
...
def plot_projection_RM(ax, chgrid, nu0, color, ls="-", alpha=0.35, label=None):
    ...
    ax.fill_between(chgrid.r_vals[valid], M_lo[valid], M_hi[valid],
                    color=color, alpha=alpha)
    ax.plot(chgrid.r_vals[valid], M_lo[valid], color=color, lw=0.8, ls=ls)
    ax.plot(chgrid.r_vals[valid], M_hi[valid], color=color, lw=0.8, ls=ls)
    if label:
        ax.plot([], [], color=color, lw=4, alpha=alpha, ls=ls, label=label)
