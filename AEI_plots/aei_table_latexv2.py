...
def build_latex_table(rows):
    ...
    lines.append(r'Model & $H/r$ & $r^{5\%}$ [r$_g$] & '
                 r'$B_{00}^{5\%}$ [G] & $\langle \Sigma_0 \rangle$ [g\,cm$^{-2}$] \\')
...
if __name__ == '__main__':
    ...
    for model_label, (alpha_B, alpha_S) in MODELS.items():
        for hor in HOR_LIST:
            ...
            # 5th percentile for r and B00 (robust to grid edges),
            # 50th percentile (median) for Sigma0
            r_min      = np.percentile(r_arr,   5)
            B00_min    = np.percentile(B00_arr,  5)
            Sigma0_med = np.percentile(S0_arr,  50)

            print(f"   -> r_p5={r_min:.2f} rg  "
                  f"B00_p5={B00_min:.2e} G  Sigma0_med={Sigma0_med:.2e}")
